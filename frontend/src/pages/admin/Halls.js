...
export default function Halls() {
  ...
  // 🔒 Check duplicate hall name (case-insensitive)
  const isDuplicateHallName = (name, excludeId = null) => {
    return halls.some(
      h =>
        h._id !== excludeId &&
  ...
  const addHall = async () => {
    ...
    // ❌ Prevent duplicate hall name
    if (isDuplicateHallName(name)) {
      setMessage("Hall with this name already exists.");
      return;
    }
  ...
  const updateHall = async hallId => {
    ...
    // ❌ Prevent duplicate hall name on update
    if (isDuplicateHallName(name, hallId)) {
      setMessage("Another hall with this name already exists.");
      return;
    }
            ...
            <div className="flex flex-wrap gap-2 flex-1">
              <input
                ...
                className={`p-2 border rounded ${
                  editingHallId === h._id ? "bg-white" : "bg-gray-100"
                }`}
              />
              <input
                name="capacity"
